...
<x-project-layout :project="$project">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <x-stat-card
            label="Açık"
            :value="$this->counts['open']"
            icon="exclamation-circle"
            color="blue"
        />
        <x-stat-card
            label="Bug"
            :value="$this->counts['bugs']"
            icon="bug-ant"
            color="red"
        />
        <x-stat-card
            label="Kritik"
            :value="$this->counts['critical']"
            icon="fire"
            color="red"
        />
    </div>
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4">
        ...
        @if ($statusFilter || $typeFilter || $priorityFilter)
            <flux:button
                variant="ghost"
                size="sm"
                wire:click="$set('statusFilter', ''); $set('typeFilter', ''); $set('priorityFilter', '')"
            >
                Temizle
            </flux:button>
        @endif
        ...
        <flux:card class="mb-6" wire:transition>
            ...
            <form wire:submit="createIssue" class="space-y-4">
                ...
                <div class="grid grid-cols-3 gap-4">
                    ...
                    <flux:select
                        wire:model="createForm.severity"
                        label="Ciddiyet"
                    >
                    ...
                    <flux:table.row wire:key="issue-{{ $issue->id }}">
                        <flux:table.cell>
                            <flux:icon
                                ...
                                variant="mini"
                                class="size-5"
                                style="color: {{ $issue->type->color() }}"
                            />
                        </flux:table.cell>
                        <flux:table.cell class="font-medium">
                            {{ $issue->title }}</flux:table.cell
                        >
                        <flux:table.cell>
                        ...
                        <flux:table.cell>
                            <flux:badge
                                size="sm"
                                :style="'background-color: ' . $issue->priority->color() . '20; color: ' . $issue->priority->color()"
                            >
                        ...
                        <flux:table.cell>
                            <flux:badge
                                size="sm"
                                :style="'background-color: ' . $issue->severity->color() . '20; color: ' . $issue->severity->color()"
                            >
                            ...
                            <flux:dropdown>
                                ...
                                <flux:menu>
                                    @foreach (IssueStatus::allowedTransitions()[$issue->status->value] ?? [] as $transition)
                                        <flux:menu.item
                                            wire:click="changeStatus('{{ $issue->id }}', '{{ $transition }}')"
                                        >
                                    ...
                                    >
                                        Düzenle</flux:menu.item
                                    >
                                    <flux:menu.item
                                        icon="trash"
                                        variant="danger"
                                        wire:click="deleteIssue('{{ $issue->id }}')"
                                        wire:confirm="Bu issue silinecek. Emin misiniz?"
                                    >
                                        Sil</flux:menu.item
                                    >
                                </flux:menu>
        ...
        >
            ...
            <form wire:submit="updateIssue" class="space-y-4 mt-4">
                ...
                <div class="flex gap-2 justify-end">
                    <flux:button
                        variant="ghost"
                        wire:click="$set('editingIssueId', null)"
                        >İptal</flux:button
                    >
                    <flux:button type="submit" variant="primary"
                        >Kaydet</flux:button
